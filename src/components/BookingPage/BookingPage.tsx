...
const API_BASE_URL = 'https://backend-ecommerce-50vz.onrender.com/api';  // Mismo backend que ServiceManagement
...
const professionalsData: ProfessionalData[] = [
    { id: 'prof-1', name: 'Dra. Nazareno', specialty: 'Masajes y Faciales' },
    { id: 'prof-2', name: 'Dr. Aaron', specialty: 'Dermatología' },
    { id: 'prof-3', name: 'Lic. Milton', specialty: 'Nutrición' },
];
...
const BookingPage: React.FC = () => {
  ...
  // Cargar servicios desde la API con el formato correcto
  useEffect(() => {
    const fetchServicesFromApi = async () => {
      ...
      const response = await fetch(`${API_BASE_URL}/services`);
        ...
        const loadedServices: Service[] = apiData.map((item: any) => ({
          id: String(item.serviceId),           // id en string
          name: item.name,
          description: item.description || 'Sin descripción.',
          price: parseFloat(item.price),
          durationMinutes: durationToMinutes(item.duration),  // Convierte formato HH:mm:ss a minutos
        ...
        setStatusMessage({
          type: 'error',
          message: error instanceof Error ? error.message : 'Error al cargar servicios.'
        });
        setServices([]);
      } finally {
